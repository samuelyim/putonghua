...
public class PuTongHuaDictFeed {

	
	public static int min = 1;
	public static int max = 13083;
	
	public static String getWord(int pos) {
		String filePath = "C:\\Users\\samuel.yim\\Documents\\_Personal\\Samuel\\Languages\\Putonghua\\sources\\files\\";
		...
		try {
			...
			FileUtility.writeFile(filePath + pos + ".txt", result);
		} catch (IOException e) {
			e.printStackTrace();
		}
	...
	public static void getAllWords() {
		for (int i=min; i<=max; i++) {
			try {
				Thread.sleep((int)(5000.0 * Math.random() + 5000));
